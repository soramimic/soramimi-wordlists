...
- 旧リストにのみ存在した駅(廃駅等)387 件は status=former・所在地/QID なしで収録した(新リストに同名・同読みが無いものだけを復元し、なんば→難波のような表記差の重複は除外)。未成駅・計画駅などWikidata の状態プロパティが不十分な駅は手動で former にしてよく、更新スクリプトは former を自動で current に戻さない
